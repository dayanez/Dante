...
#ifndef TNT_DANTE_SWAPCHAIN_H
#define TNT_DANTE_SWAPCHAIN_H
 ...
 * native object, it is given to dante as a `void *`, which must be of the proper type
 * for each platform dante is running on.
 *
 * \code
 ...
 * SDL_SysWMinfo wmi;
 * SDL_VERSION(&wmi.version);
 * DANTE_CHECK_POSTCONDITION(SDL_GetWindowWMInfo(sdlWindow, &wmi)) << "SDL version unsupported!";
 ...
 * @see Engine
 */
class UTILS_PUBLIC SwapChain : public DanteAPI {
public:
     ...
     * flag is not supported when rendering multiple Dante Views into this SwapChain.
     *
     ...
     *
     * @param engine A pointer to the dante Engine
     ...
     * callback on the Metal completion handler thread (as opposed to the main Dante thread).
     ...
     * of a frame after Dante has finished its processing.
     *
     ...
     * =============
     *
     * With the Metal backend, this callback signifies that Dante has completed all CPU-side
     * processing for a frame and the frame is ready to be scheduled for presentation.
     *
     * Typically, Dante is responsible for scheduling the frame's presentation to the SwapChain.
     ...
     * backend::PresentCallable passed to the callback function. In this mode, Dante will *not*
     ...
     * called before shutting down the Dante Engine. You can guarantee this by calling
     ...
     * =======================================
     *
     * On other backends, this callback serves as a notification that Dante has completed all
     * CPU-side processing for a frame. Dante proceeds with its normal presentation logic
     ...
     *
     * If handler is nullptr, the callback is guaranteed to be called on the main Dante thread.
     *
     ...
     *
     * @remark Only Dante's Metal backend supports frame callbacks. Other backends ignore the
