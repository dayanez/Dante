...
#ifndef TNT_DANTE_STREAM_H
#define TNT_DANTE_STREAM_H
...
/**
 * Stream is used to attach a video stream to a Dante `Texture`.
 *
 ...
 * or video application that uses Dante:
 *
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 * while (true) {
 *
 ...
 * - Dante invokes low-level graphics commands on the \em{driver thread}.
 * - The thread that calls `beginFrame` is called the \em{main thread}.
 *
 * For ACQUIRED streams, there is no need to perform the copy because Dante explicitly acquires
 ...
 */
class UTILS_PUBLIC Stream : public DanteAPI {
...
public:
    ...
    public:
         ...
         *
         * @param engine Reference to the dante::Engine to associate this Stream with.
         *
     ...
     *
     * This method tells Dante to immediately "acquire" the image and trigger a callback
     ...
     * @param callback   This is triggered by Dante when it wishes to release the image.
     *                   The callback tales two arguments: the AHardwareBuffer and the userdata.
     * @param userdata   Optional closure data. Dante will pass this into the callback when it
     *                   releases the image.
