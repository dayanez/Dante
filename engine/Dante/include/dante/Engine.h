...
#ifndef TNT_DANTE_ENGINE_H
#define TNT_DANTE_ENGINE_H
...
#ifndef DANTE_PER_RENDER_PASS_ARENA_SIZE_IN_MB
#    define DANTE_PER_RENDER_PASS_ARENA_SIZE_IN_MB 3
#endif

#ifndef DANTE_PER_FRAME_COMMANDS_SIZE_IN_MB
#    define DANTE_PER_FRAME_COMMANDS_SIZE_IN_MB 2
#endif

#ifndef DANTE_MIN_COMMAND_BUFFERS_SIZE_IN_MB
#    define DANTE_MIN_COMMAND_BUFFERS_SIZE_IN_MB 1
#endif

#ifndef DANTE_COMMAND_BUFFER_SIZE_IN_MB
#    define DANTE_COMMAND_BUFFER_SIZE_IN_MB (DANTE_MIN_COMMAND_BUFFERS_SIZE_IN_MB * 3)
#endif

/**
 * Engine is dante's main entry-point.
 *
 ...
 *
 * To use dante, an Engine instance must be created first:
 *
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ...
 * window is managed by a dante.Renderer.
 *
 * A typical dante render loop looks like this:
 *
 *
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 * #include <dante/Engine.h>
 ...
 * or a view. Because a SwapChain is initialized from a native object, it is given to dante
 * as a `void*`, which must be of the proper type for each platform dante is running on.
 *
 * @see SwapChain
 *
 *
...
public:
    ...
    struct Config {
         ...
         */
        uint32_t commandBufferSizeMB = DANTE_COMMAND_BUFFER_SIZE_IN_MB;
         ...
         * This value affects the application's memory usage.
         */
        uint32_t perRenderPassArenaSizeMB = DANTE_PER_RENDER_PASS_ARENA_SIZE_IN_MB;
         ...
         */
        uint32_t minCommandBufferSizeMB = DANTE_MIN_COMMAND_BUFFERS_SIZE_IN_MB;
         ...
         * This value does not affect the application's memory usage.
         */
        uint32_t perFrameCommandsSizeMB = DANTE_PER_FRAME_COMMANDS_SIZE_IN_MB;
        ...
        /**
         * When uploading vertex or index data, the Dante Metal backend copies data
        ...
        /*
         * Sets a preferred shader language for Dante to use.
         *
         ...
         *
         * By default, when preferredShaderLanguage is unset, Dante will prefer METAL_LIBRARY
         * shaders if present within a material, falling back to MSL. Setting
         * preferredShaderLanguage to ShaderLanguage::MSL will instead instruct Dante to check
    ...
    /**
     * Engine::Builder is used to create a new dante Engine.
     */
    ...
    public:
         ...
         *
         *                 All methods of this interface are called from dante's
         ...
         *                      when creating dante's internal context.
         *
         * @return A reference to this Builder for chaining calls.
         */
        ...
        /**
         * @param featureLevel The feature level at which initialize Dante.
         * @return A reference to this Builder for chaining calls.
         */
...
#if UTILS_HAS_THREADING
        /**
         * Creates the dante Engine asynchronously.
         *
     ...
     * it ensures all dante resources are freed.
     *
     ...
     *
     * @param engine A pointer to the dante.Engine* to be destroyed.
    ...
    /**
     * Queries the maximum number of GPU instances that Dante creates when automatic instancing
    ...
    /**
     * Returns the maximum number of stereoscopic eyes supported by Dante. The actual number of
    ...
    /**
     * @return EntityManager used by dante
     */
    ...
    void destroy(utils::Entity e);    //!< Destroys all dante-known components from this entity
     ...
     * This is useful because otherwise callbacks will be executed by dante at a later time,
     ...
     *
     * When Engine::create is called with no platform argument, Dante creates an appropriate
     ...
     * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     *
     * When a custom Platform is passed to Engine::create, Dante will use it instead, and this
    ...
    /**
      ...
      *
      * @return JobSystem used by dante
      */
    ...
    /**
      ...
      *
      * This is only useful when integrating an external renderer into Dante on platforms
      * like WebGL, where share contexts do not exist. Dante keeps track of the GL
      ...
      * state in the mean time, Dante will use that new state unknowingly.
      *
