...
static Status processParameter(MaterialBuilder& builder, const JsonishObject& jsonObject) noexcept {
    ...
    using dante::backend::ShaderStageFlags;
    ...
    if (stagesValue) {
        ...
        for (auto value: stagesValue->toJsonArray()->getElements()) {
            if (value->getType() == JsonishValue::Type::STRING) {
                ...
                using Qualifier = dante::BufferInterfaceBlock::Qualifier;
...
static Status processBufferField(dante::BufferInterfaceBlock::Builder& builder,
...
static Status processBuffer(MaterialBuilder& builder,
    ...
    dante::BufferInterfaceBlock::Builder bibb;

    bibb.target(dante::BufferInterfaceBlock::Target::SSBO);
    bibb.alignment(dante::BufferInterfaceBlock::Alignment::std430);
...
static Status processRequires(MaterialBuilder& builder, const JsonishValue& value) {
    using Attribute = dante::VertexAttribute;
...
static Status processFeatureLevel(MaterialBuilder& builder, const JsonishValue& value) {
    using dante::backend::FeatureLevel;
...
static Status processGroupSizes(MaterialBuilder& builder, const JsonishValue& v) {
    ...
    dante::math::uint3 groupSize{ 1, 1, 1 };
    size_t index = 0;
...
static Status processVariantFilter(MaterialBuilder& builder, const JsonishValue& value) {
    ...
    using dante::Variant;
    static const std::unordered_map<std::string_view, dante::UserVariantFilterBit> strToEnum  = [] {
        std::unordered_map<std::string_view, dante::UserVariantFilterBit> strToEnum;
