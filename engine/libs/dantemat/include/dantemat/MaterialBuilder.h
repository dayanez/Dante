...
#ifndef TNT_DANTEMAT_MATERIAL_PACKAGE_BUILDER_H
#define TNT_DANTEMAT_MATERIAL_PACKAGE_BUILDER_H
...
public:
     ...
     * If linking against dantemat_lite, only `OPENGL` is allowed.
     */
     ...
     * If linking against dantemat_lite, this _must_ be called with Optimization::NONE.
     */
...
private:
    ...
    bool generateShaders(
            ...
            const std::vector<dantemat::Variant>& variants, ChunkContainer& container,
...
template<>
struct utils::EnableBitMaskOperators<dantemat::MaterialBuilder::TargetApi> : std::true_type {
};

template<>
struct utils::EnableBitMaskOperators<dantemat::MaterialBuilder::Workarounds> : std::true_type {
};
