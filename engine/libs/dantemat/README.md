...
Dantemat allows for generating materials programatically on the device as opposed to with the `matc`
tool on the host machine. The cost is a binary size increase of your app due to the relatively
larger size of the `dantemat` library.

For a smaller-sized library, see [`dantemat_lite`](#dantemat-lite). It has no dependencies on
`glslang`, but can only compile materials for OpenGL and does no shader code optimization.

The dantemat package is included in the releases available on
[GitHub](https://github.com/google/filament/releases).

## Libraries

Dantemat is distributed as a set of static libraries you must link against:

- `dantemat`, Dantemat main library
- `dantebridge`, Support library for Filament / Dantemat
- `shaders`, Shader text for material generation
- `utils`, Support library for Filament / Dantemat
- `smol-v`, SPIR-V compression library

To use Dantemat from Java you must use the following two libraries instead:
- `dantemat-java.jar`, Contains Dantemat's Java classes
- `dantemat-jni`, Dantemat's JNI bindings

## Linking against Dantemat

This walkthrough will get you successfully compiling and linking native code against Dantemat with
...
We'll use a platform-specific Makefile to compile and link `main.cpp` with Dantemat's libraries.
Copy your platform's Makefile below into a `Makefile` inside the same directory.

### Linux

```make
FILAMENT_LIBS=-ldantemat -ldantebridge -lshaders -lutils -lsmol-v
CC=clang++
...
When building Dantemat from source, the `USE_STATIC_CRT` CMake option can be
used to change the run-time library version.

```make
FILAMENT_LIBS=lib/x86_64/mt/dantemat.lib lib/x86_64/mt/dantebridge.lib lib/x86_64/mt/shaders.lib \
...
addition to Dantemat's.

## Dantemat Lite

The `dantemat_lite` library is interchangeable with `dantemat`, with a few caveats:
...
In addition, `dantemat_lite` only performs a simple text match to determine which properties on the
