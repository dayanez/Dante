...
#include <danteapp/IcoSphere.h>
#include <danteapp/Sphere.h>
...
struct Geometry {
    ...
    std::vector<dante::math::short4> tangents;
    dante::VertexBuffer* vertexBuffer = nullptr;
    dante::IndexBuffer* indexBuffer = nullptr;
};
...
Sphere::Sphere(Engine& engine, Material const* material, bool culling)
    ...
    if (geometry == nullptr) {
        ...
        geometry->vertexBuffer->setBufferAt(engine, 1,
                VertexBuffer::BufferDescriptor(geometry->tangents.data(), geometry->tangents.size() * sizeof(dante::math::short4)));
...
Sphere& Sphere::setPosition(dante::math::float3 const& position) noexcept {
