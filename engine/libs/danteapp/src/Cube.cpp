...
const dante::math::float3 Cube::mVertices[] = {
...
Cube::Cube(Engine& engine, dante::Material const* material, float3 linearColor, bool culling) :
        mEngine(engine),
        mMaterial(material) {
...
void Cube::mapFrustum(dante::Engine& engine, Camera const* camera) {
...
void Cube::mapFrustum(dante::Engine& engine, dante::math::mat4 const& transform) {
    // the Camera far plane is at infinity, but we want it closer for display
    mat4f p(transform);
...
void Cube::mapAabb(dante::Engine& engine, dante::Box const& box) {
