...
DanteApp& DanteApp::get() {
    static DanteApp danteApp;
    return danteApp;
}
...
View* DanteApp::getGuiView() const noexcept { return mImGuiHelper->getView(); }

void DanteApp::run(Config const& config, SetupCallback setupCallback,
    ...
    Engine::Backend backend = dante::app::resolveBackend(config.backend);

    backend::Platform* platform = nullptr;
    if (backend == Engine::Backend::VULKAN) {
        platform = mVulkanPlatform =
                dante::app::createVulkanPlatform(config.vulkanGPUHint.c_str());
    } else if (backend == Engine::Backend::WEBGPU) {
        platform = mWebGPUPlatform =
                dante::app::createWebGPUPlatform(config.forcedWebGPUBackend);
    }

    Engine::Config engineConfig = {
#if defined(DANTE_SAMPLES_STEREO_TYPE_INSTANCED)
        .stereoscopicType = Engine::StereoscopicType::INSTANCED,
#elif defined(DANTE_SAMPLES_STEREO_TYPE_MULTIVIEW)
    ...
    // Dante. Users should manage this flag at their discretion.
    ...
    mWindow.reset(
            new DanteApp::Window(this, config, config.title, mCameraParams, width, height));
    Window* window = mWindow.get();

    mDepthMaterial = Material::Builder()
            .package(DANTEAPP_DEPTHVISUALIZER_DATA, DANTEAPP_DEPTHVISUALIZER_SIZE)
            .build(*mEngine);

    mDepthMI = mDepthMaterial->createInstance();

    mDefaultMaterial = Material::Builder()
            .package(DANTEAPP_AIDEFAULTMAT_DATA, DANTEAPP_AIDEFAULTMAT_SIZE)
            .build(*mEngine);

    mTransparentMaterial = Material::Builder()
            .package(DANTEAPP_TRANSPARENTCOLOR_DATA, DANTEAPP_TRANSPARENTCOLOR_SIZE)
            .build(*mEngine);
...
try {
    ...
    std::vector<dante::app::AppEvent> events;
        ...
        if (mImGuiHelper) {
            ...
            switch (event.type) {
                ...
                case AppEvent::Type::KEYUP: {
                    ...
                    io.AddKeyEvent(danteapp_utils::AppKeyToImGuiKey(event.key.code),
    ...
    for (const auto& event: events) {
        ...
        switch (event.type) {
            ...
            case AppEvent::Type::RESIZED:
                ...
                // Call the resize callback, if this DanteApp has one. This must be done
    ...
    // Populate the UI scene, regardless of whether Dante wants to a skip frame. We should
    ...
    if (renderer->beginFrame(window->getSwapChain())) {
        for (dante::View* offscreenView: mOffscreenViews) {
    ...
    LOG(ERROR) << "Dante exception (terminate cleanly): " << e.what();
...
void DanteApp::shutdown() {
    ...
    if (mVulkanPlatform) {
        dante::app::destroyVulkanPlatform(mVulkanPlatform);
        mVulkanPlatform = nullptr;
    }
    if (mWebGPUPlatform) {
        dante::app::destroyWebGPUPlatform(mWebGPUPlatform);
...
void DanteApp::loadIBL(std::string_view path) {
    Path iblPath(path);
    if (!iblPath.exists()) {
...
void DanteApp::loadIBL(const Config& config) {
...
void DanteApp::loadDirt(const Config& config) {
...
void DanteApp::setCameraFrustumEnabled(bool enabled) noexcept {
    mCameraFrustumEnabled = enabled ? 0x2 : 0x0;
}

void DanteApp::setDirectionalShadowFrustumEnabled(bool enabled) noexcept {
    mDirectionalShadowFrustumEnabled = enabled ? 0x2 : 0x0;
}

void DanteApp::setFroxelGridEnabled(bool enabled) noexcept {
    mFroxelGridEnabled = enabled ? 0x3 : 0x0;
}

bool DanteApp::isCameraFrustumEnabled() const noexcept {
    return !!mCameraFrustumEnabled;
}

bool DanteApp::isDirectionalShadowFrustumEnabled() const noexcept {
    return !!mDirectionalShadowFrustumEnabled;
}

bool DanteApp::isFroxelGridEnabled() const noexcept {
    return !!mFroxelGridEnabled;
}

// ------------------------------------------------------------------------------------------------

DanteApp::Window::Window(DanteApp* danteApp, const Config& config, std::string title,
        WindowCameraParams const& cameraParams, size_t w, size_t h)
        : mDisplayManager(danteApp->mDisplayManager),
          mEngine(danteApp->mEngine),
          mConfig(config),
    ...
    if (config.headless) {
        ...
        mSwapChain = engine->createSwapChain(nativeWindow,
                dante::SwapChain::CONFIG_HAS_STENCIL_BUFFER);
    }
    danteApp->mPrimarySwapChain = mSwapChain;
...
void DanteApp::Window::mouseDown(int button, ssize_t x, ssize_t y) {
...
void DanteApp::Window::mouseWheel(ssize_t x) {
...
void DanteApp::Window::mouseUp(ssize_t x, ssize_t y) {
    fixupMouseCoordinatesForHdpi(x, y);
    if (mMouseEventTarget) {
        y = mHeight - y;
...
void DanteApp::Window::mouseMoved(ssize_t x, ssize_t y) {
    fixupMouseCoordinatesForHdpi(x, y);
    y = mHeight - y;
    if (mMouseEventTarget) {
...
void DanteApp::Window::keyDown(AppKey key) {
...
void DanteApp::Window::keyUp(AppKey key) {
    auto& eventTarget = mKeyEventTarget[key];
    if (!eventTarget) {
        return;
    }
...
void DanteApp::Window::fixupMouseCoordinatesForHdpi(ssize_t& x, ssize_t& y) const {
...
DanteApp::CView::CView(Renderer& renderer, std::string name)
        : engine(*renderer.getEngine()), mName(name) {
    view = engine.createView();
    view->setName(name.c_str());
}

DanteApp::CView::~CView() {
    engine.destroy(view);
}

void DanteApp::CView::setViewport(dante::Viewport const& viewport) {
    mViewport = viewport;
...
void DanteApp::CView::mouseDown(int button, ssize_t x, ssize_t y) {
    if (mCameraManipulator) {
        mCameraManipulator->grabBegin(x, y, button == 3);
    }
}

void DanteApp::CView::mouseUp(ssize_t x, ssize_t y) {
    if (mCameraManipulator) {
        mCameraManipulator->grabEnd();
    }
}

void DanteApp::CView::mouseMoved(ssize_t x, ssize_t y) {
    if (mCameraManipulator) {
        mCameraManipulator->grabUpdate(x, y);
    }
}

void DanteApp::CView::mouseWheel(ssize_t x) {
    if (mCameraManipulator) {
        mCameraManipulator->scroll(0, 0, x);
    }
}

bool DanteApp::manipulatorKeyFromKeycode(AppKey scancode, CameraManipulator::Key& key) {
    switch (scancode) {
        case AppKey::W:
...
void DanteApp::CView::keyUp(AppKey scancode) {
    if (mCameraManipulator) {
...
void DanteApp::CView::keyDown(AppKey scancode) {
    if (mCameraManipulator) {
        CameraManipulator::Key key;
        if (manipulatorKeyFromKeycode(scancode, key)) {
...
bool DanteApp::CView::intersects(ssize_t x, ssize_t y) {
...
void DanteApp::CView::setCameraManipulator(CameraManipulator* cm) {
    mCameraManipulator = cm;
}

void DanteApp::CView::setCamera(Camera* camera) {
    view->setCamera(camera);
}

void DanteApp::GodView::setGodCamera(Camera* camera) {
