...
DanteApp::Window::Handle SDLDisplayManager::createWindow(const char* title, uint32_t w,
...
#if defined(__APPLE__)
    if (mConfig.backend == dante::Engine::Backend::METAL ||
            mConfig.backend == dante::Engine::Backend::VULKAN ||
            mConfig.backend == dante::Engine::Backend::WEBGPU) {
        nativeWindow = ::setUpMetalLayer(nativeWindow);
    }
#endif

    mNativeWindowMap[window] = nativeWindow;
    return (DanteApp::Window::Handle) window;
}

void SDLDisplayManager::destroyWindow(DanteApp::Window::Handle window) {
    SDL_DestroyWindow((SDL_Window*) window);
}

void* SDLDisplayManager::getNativeWindow(DanteApp::Window::Handle window) const {
    assert_invariant(mNativeWindowMap.count(window) > 0);
    return mNativeWindowMap[window];
}

void SDLDisplayManager::setWindowTitle(DanteApp::Window::Handle window, const char* title) {
    SDL_SetWindowTitle((SDL_Window*) window, title);
}

void SDLDisplayManager::getWindowSize(DanteApp::Window::Handle window, uint32_t* w,
        uint32_t* h) const {
    int iw, ih;
...
void SDLDisplayManager::getDrawableSize(DanteApp::Window::Handle window, uint32_t* w,
        uint32_t* h) const {
    int iw, ih;
...
bool SDLDisplayManager::isWindowFocused(DanteApp::Window::Handle window) const {
...
void SDLDisplayManager::onWindowResized(DanteApp::Window::Handle window) {
#if defined(__APPLE__)
