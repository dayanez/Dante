...
FDanteInstance::FDanteInstance(Entity root, FDanteAsset const* owner) :
    mRoot(root),
    mOwner(owner),
    mNodeMap(owner->mSourceAsset->hierarchy->nodes_count, Entity()) {}

FDanteInstance::~FDanteInstance() {
    delete mAnimator;
...
Animator* FDanteInstance::getAnimator() const noexcept {
    assert_invariant(mAnimator);
    return mAnimator;
}

void FDanteInstance::createAnimator() {
    if (mAnimator == nullptr && mOwner->mResourcesLoaded) {
        mAnimator = new Animator(mOwner, this);
    }
}

size_t FDanteInstance::getSkinCount() const noexcept {
    return mSkins.size();
}

const char* FDanteInstance::getSkinNameAt(size_t skinIndex) const noexcept {
...
size_t FDanteInstance::getJointCountAt(size_t skinIndex) const noexcept {
    if (mSkins.size() <= skinIndex) {
        return 0;
    }
...
const utils::Entity* FDanteInstance::getJointsAt(size_t skinIndex) const noexcept {
    if (mSkins.size() <= skinIndex) {
        return nullptr;
    }
...
void FDanteInstance::attachSkin(size_t skinIndex, Entity target) noexcept {
...
void FDanteInstance::detachSkin(size_t skinIndex, Entity target) noexcept {
    if (UTILS_UNLIKELY(mSkins.size() <= skinIndex || target.isNull())) {
        return;
    }
...
mat4f const* FDanteInstance::getInverseBindMatricesAt(size_t skinIndex) const {
    assert_invariant(mOwner);
    DANTE_CHECK_PRECONDITION(skinIndex < mOwner->mSkins.size())
            << "skinIndex must be less than the number of skins in this instance.";
    return mOwner->mSkins[skinIndex].inverseBindMatrices.data();
}

void FDanteInstance::recomputeBoundingBoxes() {
    DANTE_CHECK_PRECONDITION(mOwner->mSourceAsset)
            << "Do not call releaseSourceData before recomputeBoundingBoxes";

    DANTE_CHECK_PRECONDITION(mOwner->mResourcesLoaded)
    ...
    auto computeBoundingBoxSkinned = [&tm, this](const Prim& prim) -> Aabb {
        ...
        const FDanteAsset::Skin& assetSkin = mOwner->mSkins[prim.skinIndex];
        for (size_t i = 0, n = verts.size(); i < n; i++) {
            ...
            // NOTE: Dante's vertex shader assumes that last row is [0,0,0,1]
...
size_t FDanteInstance::getMaterialVariantCount() const noexcept {
    return mVariants.size();
}

const char* FDanteInstance::getMaterialVariantName(size_t variantIndex) const noexcept {
...
void FDanteInstance::applyMaterialVariant(size_t variantIndex) noexcept {
    if (variantIndex >= mVariants.size()) {
        return;
    }
...
void DanteInstance::detachMaterialInstances() {
    downcast(this)->detachMaterialInstances();
}

size_t DanteInstance::getMaterialInstanceCount() const noexcept {
    return downcast(this)->getMaterialInstanceCount();
}

const MaterialInstance* const* DanteInstance::getMaterialInstances() const noexcept {
    return downcast(this)->getMaterialInstances();
}

MaterialInstance* const* DanteInstance::getMaterialInstances() noexcept {
    return downcast(this)->getMaterialInstances();
}

const char* DanteInstance::getMaterialVariantName(size_t variantIndex) const noexcept {
    return downcast(this)->getMaterialVariantName(variantIndex);
}

void DanteInstance::applyMaterialVariant(size_t variantIndex) noexcept {
    return downcast(this)->applyMaterialVariant(variantIndex);
}

size_t DanteInstance::getMaterialVariantCount() const noexcept {
    return downcast(this)->getMaterialVariantCount();
}

DanteAsset const* DanteInstance::getAsset() const noexcept {
    return downcast(this)->mOwner;
}

size_t DanteInstance::getEntityCount() const noexcept {
    return downcast(this)->mEntities.size();
}

const Entity* DanteInstance::getEntities() const noexcept {
    const auto& entities = downcast(this)->mEntities;
    return entities.empty() ? nullptr : entities.data();
}

Entity DanteInstance::getRoot() const noexcept {
    return downcast(this)->mRoot;
}

Animator* DanteInstance::getAnimator() noexcept {
    return downcast(this)->getAnimator();
}

size_t DanteInstance::getSkinCount() const noexcept {
    return downcast(this)->getSkinCount();
}

const char* DanteInstance::getSkinNameAt(size_t skinIndex) const noexcept {
    return downcast(this)->getSkinNameAt(skinIndex);
}

size_t DanteInstance::getJointCountAt(size_t skinIndex) const noexcept {
    return downcast(this)->getJointCountAt(skinIndex);
}

const Entity* DanteInstance::getJointsAt(size_t skinIndex) const noexcept {
    return downcast(this)->getJointsAt(skinIndex);
}

void DanteInstance::attachSkin(size_t skinIndex, Entity target) noexcept {
    return downcast(this)->attachSkin(skinIndex, target);
}

void DanteInstance::detachSkin(size_t skinIndex, Entity target) noexcept {
    return downcast(this)->detachSkin(skinIndex, target);
}

math::mat4f const* DanteInstance::getInverseBindMatricesAt(size_t skinIndex) const {
    return downcast(this)->getInverseBindMatricesAt(skinIndex);
}

void DanteInstance::recomputeBoundingBoxes() {
    return downcast(this)->recomputeBoundingBoxes();
}

Aabb DanteInstance::getBoundingBox() const noexcept {
