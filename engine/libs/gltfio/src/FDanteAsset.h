...
#ifndef GLTFIO_FDANTEASSET_H
#define GLTFIO_FDANTEASSET_H
...
#if defined(__EMSCRIPTEN__) || defined(__ANDROID__) || defined(DANTE_IOS)
...
// Since material instances are not typically shared between DanteInstance, the slots are a
...
// MeshCache
// ---------
// If a given glTF mesh is referenced by multiple glTF nodes, then it generates a separate Dante
...
// "primitive" is a reference to a Dante VertexBuffer and IndexBuffer.
struct Primitive {
...
struct FDanteAsset : public DanteAsset {
    struct ResourceInfo;
    struct ResourceInfoExtended;

    FDanteAsset(Engine* engine, utils::NameComponentManager* names,
    ...
    DanteInstance** getAssetInstances() noexcept {
        return (DanteInstance**) mInstances.data();
    }
    ...
    // If a Dante Texture for the given args already exists, calls setParameter() and returns
    ...
    Aabb mBoundingBox;
    utils::Entity mRoot;
    std::vector<FDanteInstance*> mInstances;
    ...
    bool mDetachedDanteComponents = false;
    ...
    //   unique Dante Texture resource.
    // A cgltf_texture can map to multiple asset textures (Dante textures) if they have different
    // color space flags (sRGB vs Linear). Note that more than one cgltf_texture can map to a single
    // Dante texture, e.g. if several have the same URL or bufferView. For each Dante texture,
...
DANTE_DOWNCAST(DanteAsset)

} // namespace dante::gltfio

#endif // GLTFIO_FDANTEASSET_H
