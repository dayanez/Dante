 ...
 *
 * ResourceLoader must be destroyed on the same thread that calls dante::Renderer::render()
 * because it listens to dante::backend::BufferDescriptor callbacks in order to determine when to
 * free CPU-side data blobs.
 *
 * \todo If clients persist their ResourceLoader, Dante textures are currently re-created upon
 * subsequent re-loads of the same asset. To fix this, we would need to enable shared ownership
 * of Texture objects between ResourceLoader and DanteAsset.
 */
...
public:
     ...
     * also DanteAsset#getResourceUris.
     *
    ...
    /**
     * Register a plugin that can consume PNG / JPEG content and produce dante::Texture objects.
     *
     ...
     */
    bool loadResources(DanteAsset* asset);
     ...
     */
    bool asyncBeginLoad(DanteAsset* asset);
...
private:
    bool loadResources(FDanteAsset* asset, bool async);
    struct Impl;
    Impl* pImpl;
};
