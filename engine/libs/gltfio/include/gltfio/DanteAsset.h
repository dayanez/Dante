...
#ifndef GLTFIO_DANTEASSET_H
#define GLTFIO_DANTEASSET_H
...
/**
 * \class DanteAsset DanteAsset.h gltfio/DanteAsset.h
 * \brief Owns a bundle of Dante objects that have been created by AssetLoader.
 *
 ...
 * a dante::TransformManager component, and some entities also have \c Name, \c Renderable,
 ...
 * dante::VertexBuffer, dante::IndexBuffer, dante::Texture,
 * and, optionally, a simple animation engine (gltfio::Animator).
 *
 * Clients must use ResourceLoader to create dante::Texture objects, compute tangent quaternions,
...
public:
     ...
     *
     * This does not return a bounding box over all DanteInstance, it's just a straightforward
     * AAAB that can be determined at load time from the asset data.
     */
    dante::Aabb getBoundingBox() const noexcept;
    ...
    /**
     * Returns the Dante engine associated with the AssetLoader that created this asset.
     */
    dante::Engine* getEngine() const noexcept;
    ...
    /**
     * Adds entities to a Dante scene only if they belong to at least one of the given glTF
     * scenes.
     *
     ...
     */
    void addEntitiesToScene(dante::Scene& targetScene, const Entity* entities, size_t count,
            SceneMask sceneFilter) const;

    /**
     * Releases ownership of entities and their Dante components.
     *
     * This makes the client take responsibility for destroying Dante
     * components (e.g. Renderable, TransformManager component) as well as
     * the underlying entities.
     */
    void detachDanteComponents() noexcept;

    bool areDanteComponentsDetached() const noexcept;

    /**
     * Convenience function to get the first instance, or null if it doesn't exist.
     */
    DanteInstance* getInstance() noexcept {
        return getAssetInstanceCount() > 0 ? getAssetInstances()[0] : nullptr;
    }

    /*! \cond PRIVATE */

    DanteInstance** getAssetInstances() noexcept;
    size_t getAssetInstanceCount() const noexcept;

protected:
    DanteAsset() noexcept = default;
    ~DanteAsset() = default;

public:
    DanteAsset(DanteAsset const&) = delete;
    DanteAsset(DanteAsset&&) = delete;
    DanteAsset& operator=(DanteAsset const&) = delete;
    DanteAsset& operator=(DanteAsset&&) = delete;
    /*! \endcond */
};

} // namespace dante::gltfio

#endif // GLTFIO_DANTEASSET_H
