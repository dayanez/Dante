...
class DanteflatSecurityTest : public ::testing::Test {
protected:
...
TEST_F(DanteflatSecurityTest, DictionaryTextOOBRead) {
    ...
    write64(fileData, (uint64_t)dantemat::ChunkType::DictionaryText);
    ...
    DictionaryReader::unflatten(container, dantemat::ChunkType::DictionaryText, dictionary);
...
TEST_F(DanteflatSecurityTest, MaterialChunkHeapOverflow) {
    std::vector<uint8_t> payload;
    ...
    std::vector<uint8_t> fileData;
    write64(fileData, (uint64_t)dantemat::ChunkType::MaterialGlsl);
    write32(fileData, payload.size());
    fileData.insert(fileData.end(), payload.begin(), payload.end());
    ...
    ASSERT_TRUE(chunk.initialize(dantemat::ChunkType::MaterialGlsl));
...
TEST_F(DanteflatSecurityTest, MaterialChunkOOBReadText) {
    std::vector<uint8_t> payload;
    write16(payload, 0); // mSharedStrings
    write16(payload, 0); // mVertexStrings
    write16(payload, 0); // mFragmentStrings
...
TEST_F(DanteflatSecurityTest, MaterialChunkOOBReadBinary) {
    std::vector<uint8_t> payload;
    write64(payload, 1); // numShaders
    payload.push_back(1); // model
    payload.push_back(0); // variant
    ...
    write64(fileData, (uint64_t)dantemat::ChunkType::MaterialSpirv);
    write32(fileData, payload.size());
    fileData.insert(fileData.end(), payload.begin(), payload.end());
    ...
    MaterialChunk chunk(container);
    ASSERT_TRUE(chunk.initialize(dantemat::ChunkType::MaterialSpirv));
...
TEST_F(DanteflatSecurityTest, UnflattenerIntegerWrapBypass) {
    std::vector<uint8_t> payload;
